...
Usage:  python one_mark_refine CONFIG_PATH
where:  CONFIG_PATH provides the path to a JSON configuration file that
        includes the following key-value pairs:
            ...
            3. modification_sequence_path : path to a file containing the
                full-resolution modification sequence for the mark
            4. output_dir (optional) : Path to the new output directory for ALL
...
chem_mods_path = np.array([config["modification_sequence_path"]])
...
binders[0].chemical_potential = config_cg["chemical_potential"]
binders[0].interaction_energy = config_cg["interaction_energy"]
...
run_command = (f"python {' '.join(sys.argv)}\n"
               f"# Refining Simulation {config['CG_output']}")
