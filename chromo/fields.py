...
from chromo.util import combine_repeat
...
class Reconstructor:
    ...
    def from_file(cls, path):
        ...
        Read info about the field as a series
        """
...
class UniformDensityField(FieldBase):
    ...
    def __init__(self, polymers, marks, x_width, nx, y_width, ny, z_width,
        ...
        The `self.density` attribute is an array with one column of densities
        for each state and one column for actual bead densities of the polymer.

        The `self._recompute_field` method initializes the density field.
        ...
        for poly in polymers:
            if poly.num_marks != len(marks):
                raise NotImplementedError(
                    "For now, all polymers must use all"
                    " of the same marks."
                )
    ...
    def compute_dE(
        self, poly, inds, r_trial, t3_trial, t2_trial, states_trial,
        continuous_inds
    ):
        """
